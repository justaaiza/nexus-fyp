...
type RubricItem = { criteria: string; score: number; max: number; comment: string; };
...
type FeedbackItem = {
  ...
  submission: { _id: string; status: string; fileUrl: string; milestone?: { title: string; deadline: string; phase: string }; };
};
...
export function StudentFeedback() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      <PageHeader title="Feedback & Grades" subtitle="View supervisor evaluations and rubric-based feedback on your submissions." />
      ...
      {feedbackItems.length === 0 ? (
        <EmptyState icon={MessageSquare} title="No feedback yet" description="Supervisor evaluations and rubric-based feedback will appear here after your submissions are graded." />
      ) : (
        <div className="space-y-4">
          {feedbackItems.map((item) => {
            const isOpen = expanded === item._id;
            const scoreVal = item.grade || 0;
            const gradeColor = scoreVal >= 90 ? "#10b981" : scoreVal >= 75 ? "#3b7fe8" : "#f59e0b";
            ...
            return (
              <div key={item._id} className="rounded-2xl overflow-hidden bg-fyp-card border border-fyp-border">
                <button className="w-full p-5 flex items-center gap-4 text-left" onClick={() => setExpanded(isOpen ? null : item._id)}>
                  {item.grade !== null ? (
                    <GradeCircle score={item.grade} max={100} />
                  ) : (
                  ...
                  <div className="border-t border-fyp-border">
                    ...
                    {item.submission?.fileUrl && (
                      <div className="px-5 pb-5">
                        <a href={`http://localhost:5000${item.submission.fileUrl}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm bg-fyp-elevated text-fyp-text-secondary border border-fyp-border w-fit">
                          <FileText size={13} /> Download Submission
                        </a>
                      </div>
                    )}
                  </div>
